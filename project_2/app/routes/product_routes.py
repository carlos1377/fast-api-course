...
router = APIRouter(prefix='/product',
                   tags=['Product'], dependencies=[Depends(auth)])


@router.post(
    '/add', status_code=status.HTTP_201_CREATED, description='Add new product')
def add_product(
...
@router.get(
    '/list', response_model=Page[ProductOutput], description='List products')
def list_products(
    search: str = '',
